...
func createTestSDENoWormholes(t *testing.T) string {
  ...
  factionID: 500001
`
	if err := os.WriteFile(tmpDir+"/mapRegions.yaml", []byte(regionsYAML), 0644); err != nil {
...
`
	if err := os.WriteFile(tmpDir+"/mapConstellations.yaml", []byte(constellationsYAML), 0644); err != nil {
...
`
	if err := os.WriteFile(tmpDir+"/mapSolarSystems.yaml", []byte(systemsYAML), 0644); err != nil {
